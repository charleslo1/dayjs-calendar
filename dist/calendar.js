...
var calendar = (function (o, c, d) {
  ...
  c.getCalendarFormat = function (mydayjs) {
    ...
    var key = diff < -6 ? 'sameElse' : diff < -1 ? 'lastWeek' : diff < 0 ? 'lastDay' : diff < 1 ? 'sameDay' : diff < 2 ? 'nextDay' : diff < 7 ? 'nextWeek' : 'sameElse';
    return options.formats[key || 'sameElse'];
  };

  c.prototype.calendar = function () {
    var format = c.getCalendarFormat(this);
